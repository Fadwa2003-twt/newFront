...
function TweeterSocial(props) {
    ...
    const accountsObject = useSelector((state) => state.accounts);
    ...
    useEffect(() => {
        dispatch(GetAccountsAction({page: 1}));
        dispatch(GetCategoriesAccountAction({page: 1}));
    }, [dispatch]);

    useEffect(() => {
        if (selectedRows && selectedRows.length > 0) {
            const accountsCategories = accounts?.filter((account) => selectedRows.includes(account.Category?._id));
            setTableData(accountsCategories);
        } else {
            setTableData(accounts); // في حال لم يتم اختيار أي صف، عرض كل الحسابات
    ...
    const handelClickDeleteBtn = () => {
        if (accountsIds.length > 0) {
            dispatch(DeleteSomeAccountsAction({accountsIds: accountsIds}));
        } else {
    ...
    useEffect(() => {
        ...
        setCheckCookesAccounts(
            accounts.filter((account) => account.AccountBasicInfo.Cookie)
        );
    ...
    const storeCookies = () => {
        if (accountsIds.length > 0) {
            ...
            dispatch(
                StoreCookiesAction({accounts: accountInfo, clear: clearCheck})
            );
        } else {
            aleartsToast("error", "يرجى اختيار بعض الحسابات ");
        }
    ...
    useEffect(() => {
        const dataFilter = accounts.filter((account) =>
    ...
    return (
        <>
            <div className={"max-w-full min-w-[300px] flex-grow flex flex-col md:flex-row gap-10 p-5 py-10"}>
                ...
                <div className={"max-w-[80%] w-[70%]"}>
                    <div className={"flex w-full gap-2 flex-wrap"}>
                        ...
                        <DefaultBtn onClick={handelRetweet} className={"w-auto bg-primary-btn text-primary rounded-md"}
                                    classBtn={"w-auto"} title={"رتويت"}/>
                        <DefaultBtn onClick={handelLike} className={"w-auto bg-primary-btn text-primary rounded-md"}
                                    classBtn={"w-auto"} title={"لايك"}/>
                        <DefaultBtn onClick={handelComment} className={"w-auto bg-primary-btn text-primary rounded-md"}
                                    classBtn={"w-auto"} title={"ريبلاي"}/>
                        ...
                        <DefaultBtn onClick={handelUnlock} className={"w-auto bg-primary-btn text-primary rounded-md"}
                                    classBtn={"w-auto"}
                                    title={isLoadingUnLock ? <Spinner/> : "الغاء قفل"}/>
                        <DefaultBtn onClick={handelshowTweetFormModal}
                                    className={"w-auto bg-primary-btn text-primary rounded-md"} classBtn={"w-auto"}
                                    title={"عرض"}/>
                                ...
                                <tr key={element._id}>
                                    ...
                                    >
                                        <div className={"flex gap-1"}>
                                            {
                                                element?.AccountDataInfo1?.image ?
                                                    <img className={"max-w-8 max-h-8 rounded-full"} src={element?.AccountDataInfo1?.image} alt={"img"}/>
                                                    : null
                                            }
                                    ...
                                    >
                                        {element.AccountDataInfo1?.Tweets || "No Data"}
                                    </td>
                                    ...
                                    >
                                        {element.AccountDataInfo1?.Followers || "No Data"}
                                    ...
                                    >
                                        {element.AccountDataInfo1?.Following || "No Data"}
                                    </td>
                                    <td
            ...
            <AddTweetsModal isModalOpen={isAddTweetModel} selectedAccounts={accountsIds} type={props.type}
                            onClose={handelAddTweet}/>
            <FollowModal isModalOpen={isFollowModal} selectedAccounts={accountsIds} onClose={handelFollow}/>
            <RetweetModal isModalOpen={isReTweetModal} selectedAccounts={accountsIds} onClose={handelRetweet}/>
            <LikeModal isModalOpen={isLikeModal} selectedAccounts={accountsIds} onClose={handelLike}/>
            <ReplyModal isModalOpen={isCommentModal} selectedAccounts={accountsIds} onClose={handelComment}/>
            <UnLockModal isModalOpen={isUnlockModal} selectedAccounts={accountsIds} onClose={handelUnlock}/>
            <ShowTweetModal isModalOpen={isShowTweetFormModal} selectedAccounts={accountsIds}
                            onClose={handelshowTweetFormModal}/>
        </>
    );
}
