...
function CreatePostsInstaAction(dispatch, { textFileTweet, images, accounts }) {
  const api = rootRoute + "/api/v1/insta/post";
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (response.status === 200) {
        aleartsToast("success", "تم اضافة البوست بنجاح");
        dispatch({
        ...
        aleartsToast("error", `خطأ لم يتم عمل البوست \n ${response.errors}`);
...
export default CreatePostsInstaAction;