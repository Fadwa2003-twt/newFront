...
import {
    DELETE_SOME_ACCOUNTS, DELETE_SOME_INSTA_ACCOUNTS,
...
function DeleteInstaAccountsAction (values) {
    const user =  JSON.parse(localStorage.getItem("user"))
    const token = user.token
    const api = rootRoute + "/api/v1/accounts/delete/insta"
    return async (dispatch) => {
        ...
        try {
            ...
            if (result) {
                ...
                dispatch({
                    type: DELETE_SOME_INSTA_ACCOUNTS,
...
export default DeleteInstaAccountsAction