...
import {
    DELETE_SOME_CATEGORIES_INSTA_ACCOUNTS,
...
function DeleteSomeCategoriesInstaAccountsAction (values) {
    const user =  JSON.parse(localStorage.getItem("user"))
    const token = user.token
    const api = rootRoute + "/api/v1/accountcategories/insta/someDelete"
    console.log(values)
    return async (dispatch) => {
        ...
        try {
            ...
            if (result) {
                ...
                dispatch({
                    type: DELETE_SOME_CATEGORIES_INSTA_ACCOUNTS,
...
export default DeleteSomeCategoriesInstaAccountsAction